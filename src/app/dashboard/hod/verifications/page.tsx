...
export default async function HodVerificationPage() {
  ...
  if (rawDeptId) {
    ...
    deptLabel = departmentData?.name || 'General Department'
  }

  let pendingLecturersQuery = supabase
    .from('profiles')
    .select(
      'id, full_name, staff_id, created_at, requested_department_id, department_id, is_verified'
    )
    .eq('role', 'lecturer')
    .eq('is_verified', false)
    .not('requested_department_id', 'is', null)
    .order('created_at', { ascending: false })

  if (rawDeptId && !isAdmin) {
    pendingLecturersQuery = pendingLecturersQuery.eq('requested_department_id', rawDeptId)
  }

  const { data: pendingLecturers, error } = await pendingLecturersQuery

  if (error) {
    console.error('❌ Failed to load pending lecturers:', error)
  }
    ...
    <div className="p-8 max-w-5xl mx-auto">
      <header className="mb-10">
        ...
        <p className="text-slate-500">
          Approve new staff access requests for {deptLabel}.
        </p>
      </header>
