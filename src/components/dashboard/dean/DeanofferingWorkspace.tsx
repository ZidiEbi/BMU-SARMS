...
import {
  approveDeanOfferingResultsAction,
  returnDeanOfferingResultsAction,
  type DeanActionState,
} from '@/app/dashboard/dean/offerings/[offeringId]/actions'
...
type OfferingRecord = {
  ...
  departments?: DepartmentRecord | DepartmentRecord[] | null
}
...
const INITIAL_ACTION_STATE: DeanActionState = {
  ok: false,
  message: '',
}

export default function DeanOfferingWorkspace({
  profile,
  offering,
  registrations,
}: {
  ...
  const [approveState, approveAction] = useActionState<DeanActionState, FormData>(
    approveDeanOfferingResultsAction,
    INITIAL_ACTION_STATE
  )
  const [returnState, returnAction] = useActionState<DeanActionState, FormData>(
    returnDeanOfferingResultsAction,
    INITIAL_ACTION_STATE
  )
  ...
  const department = Array.isArray(offering.departments) ? offering.departments[0] : offering.departments
  ...
  const filteredRows = useMemo(() => {
    ...
    const sorted = [...normalizedRows].sort((a, b) => {
      ...
      if (a.status === 'HOD_APPROVED' && b.status !== 'HOD_APPROVED') return -1
      if (a.status !== 'HOD_APPROVED' && b.status === 'HOD_APPROVED') return 1
  ...
  const canApprove =
    ...
    metrics.hodApproved === metrics.totalStudents
  ...
  const statusBanner = useMemo<{
    ...
    if (metrics.locked > 0) {
      return {
        tone: 'violet',
        title: 'This offering contains locked rows',
        body: 'Some result rows are already beyond Dean control.',
      }
    }
    ...
    if (metrics.missing > 0) {
      return {
        tone: 'red',
        title: 'Dean approval blocked by missing results',
    ...
    if (issueSummary.length > 0) {
      return {
        tone: 'amber',
        title: 'Dean approval blocked by flagged issues',
        body: 'Resolve all flagged result issues before completing Dean approval.',
      }
    }

    if (metrics.hodApproved !== metrics.totalStudents) {
      return {
        tone: 'blue',
        title: 'Waiting for full HOD approval',
        body: `Only ${metrics.hodApproved} of ${metrics.totalStudents} row(s) are currently HOD_APPROVED.`,
      }
    }

    return {
      tone: 'green',
      title: 'This offering is ready for Dean approval',
      body: 'All rows are valid and already HOD_APPROVED.',
    }
    ...
    <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2 text-sm text-slate-600">
                <span className="rounded-full bg-slate-100 px-3 py-1">
                  Faculty Unit: {department?.name ?? 'Department'}
                </span>
                <span className="rounded-full bg-slate-100 px-3 py-1">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.3fr_0.7fr]">
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <StatusChip label="HOD Approved" value={metrics.hodApproved} tone="blue" />
            <StatusChip label="Dean Approved" value={metrics.deanApproved} tone="green" />
          ...
          <div className="mt-6 rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
            {canApprove ? (
              <p className="font-medium text-green-700">
                All registered rows are valid and already marked HOD_APPROVED. This offering is ready for Dean approval.
              </p>
            ) : (
              <p className="font-medium text-amber-700">
                Dean approval is blocked until every registered row is complete, issue-free, and already HOD_APPROVED.
              </p>
            )}
          </div>
        </div>
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-4 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-lg font-bold text-slate-900">Registered Students & Result Rows</h2>
            <p className="text-sm text-slate-600">
              This table is read-first and intended for faculty-level supervision before final Dean action.
            </p>
          </div>
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
          <div className="space-y-3">
            <div>
              <h2 className="text-lg font-bold text-slate-900">Dean Actions</h2>
              <p className="text-sm text-slate-600">
                Dean actions operate on the whole offering batch at faculty supervisory level.
              </p>
            </div>
            ...
            <textarea
              ...
              placeholder="Optional Dean note when returning this batch to HOD"
...
function ActionMessage({ state }: { state: DeanActionState }) {
...
function ApproveSubmitButton({ disabled }: { disabled: boolean }) {
  ...
  return (
    ...
    >
      {pending ? 'Processing...' : 'Dean Approve'}
    </button>
  )
}
...
function ReturnSubmitButton() {
  ...
  return (
    ...
    >
      {pending ? 'Processing...' : 'Return to HOD'}
    </button>
  )
}
