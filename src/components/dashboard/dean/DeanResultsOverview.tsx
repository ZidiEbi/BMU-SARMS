...
export default function DeanResultsOverview({
  offerings,
}: {
    ...
    <section className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
      <div className="p-6 border-b border-slate-100">
        ...
        <p className="text-slate-500 text-sm font-medium mt-1">
          Review HOD-approved and Dean-approved result rows within your faculty.
        </p>
      </div>
      ...
      <div className="divide-y divide-slate-100">
        {populatedOfferings.length === 0 ? (
          ...
          populatedOfferings.map((offering) => {
            ...
            const hodApprovedCount = registrations.filter((registration) => {
              const result = registration.results?.[0]
              return result?.status === 'HOD_APPROVED'
            }).length

            const deanApprovedCount = registrations.filter((registration) => {
              const result = registration.results?.[0]
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-slate-500 font-medium">
                      {offering.departments?.name ?? 'Department'} • {offering.session} • {offering.semester} • {offering.level} Level • {offering.courses?.unit ?? 0} Unit(s)
                    </p>
                  </div>
